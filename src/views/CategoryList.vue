<template>
    <div class="yb-category-list">
        <el-breadcrumb>
            <el-breadcrumb-item to="/main">首页</el-breadcrumb-item>
            <el-breadcrumb-item>菜品类别管理</el-breadcrumb-item>
            <el-breadcrumb-item>菜品类别列表</el-breadcrumb-item>
        </el-breadcrumb>
        <br>
    ...
    methods: {
        ...
        updateCategory(c,i){
            ...
            this.$prompt('请输入您想修改的类别名:','提示',{inputValue:c.cname}).then(({value})=>{
                ...
                this.$axios.put(url,{cname:value,cid:i+1}).then((res)=>{
                    if(res.data.code==200){
                        //数据库中添加成功
                        this.$message.success('类别修改成功')
                        
                    }else{
                        this.$message.error('类别修改过程出错:'+res.data.msg)
                    }
                })
            }).catch((err)=>{
                console.log(err)
